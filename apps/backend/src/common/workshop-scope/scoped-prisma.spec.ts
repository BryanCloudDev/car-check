...
describe('ScopedPrisma', () => {
  ...
  describe('customer.findMany', () => {
    it('injects workshopId when called with no args', async () => {
      ...
      expect(prisma.customer.findMany).toHaveBeenCalledWith(
        expect.objectContaining({ where: expect.objectContaining({ workshopId: WORKSHOP_A }) }),
      );
    });
    ...
    it('merges caller where with workshopId', async () => {
      ...
      expect(prisma.customer.findMany).toHaveBeenCalledWith(
        expect.objectContaining({
          where: expect.objectContaining({ workshopId: WORKSHOP_A, name: 'John' }),
        }),
      );
    });

    it('overrides any workshopId the caller passes', async () => {
      await scoped.customer.findMany({ where: { workshopId: WORKSHOP_B } as any });
    ...
    it('passes through extra args (orderBy, skip, take)', async () => {
      await scoped.customer.findMany({ orderBy: { name: 'asc' }, skip: 10, take: 5 });
      expect(prisma.customer.findMany).toHaveBeenCalledWith(
  ...
  describe('customer.findFirstOrThrow', () => {
    it('injects workshopId into where', async () => {
      ...
      expect(prisma.customer.findFirstOrThrow).toHaveBeenCalledWith(
        expect.objectContaining({
          where: expect.objectContaining({ id: 'cust-1', workshopId: WORKSHOP_A }),
        }),
      );
    });

    it('overrides a rogue workshopId from caller', async () => {
      await scoped.customer.findFirstOrThrow({ where: { workshopId: WORKSHOP_B } as any });
      const { where } = (prisma.customer.findFirstOrThrow as jest.Mock).mock.calls[0][0];
      expect(where.workshopId).toBe(WORKSHOP_A);
    });
  });
  ...
  describe('customer.create', () => {
    it('injects workshopId into data', async () => {
      ...
      expect(prisma.customer.create).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({ name: 'Ana', phone: '555', workshopId: WORKSHOP_A }),
        }),
      );
    });

    it('cannot be overridden by caller data', async () => {
      await scoped.customer.create({ data: { name: 'Ana', phone: '555', workshopId: WORKSHOP_B } as any });
      const { data } = (prisma.customer.create as jest.Mock).mock.calls[0][0];
  ...
  describe('workOrder.findMany', () => {
    ...
    it('merges caller where with workshopId', async () => {
      ...
      expect(prisma.workOrder.findMany).toHaveBeenCalledWith(
        expect.objectContaining({
          where: expect.objectContaining({ workshopId: WORKSHOP_A, status: 'RECIBIDO' }),
        }),
      );
    });

    it('overrides any workshopId the caller passes', async () => {
      await scoped.workOrder.findMany({ where: { workshopId: WORKSHOP_B } as any });
      const { where } = (prisma.workOrder.findMany as jest.Mock).mock.calls[0][0];
      expect(where.workshopId).toBe(WORKSHOP_A);
    });
  });
  ...
  describe('workOrder.findFirstOrThrow', () => {
    it('injects workshopId into where', async () => {
      ...
      expect(prisma.workOrder.findFirstOrThrow).toHaveBeenCalledWith(
        expect.objectContaining({
          where: expect.objectContaining({ id: 'wo-1', workshopId: WORKSHOP_A }),
        }),
      );
    });
  });
  ...
  describe('workOrder.create', () => {
    ...
    it('cannot be overridden by caller data', async () => {
      await scoped.workOrder.create({
        data: { vehicleId: 'v-1', customerId: 'c-1', workshopId: WORKSHOP_B } as any,
      });
  ...
  describe('workOrder.update', () => {
    it('scopes where to the correct workshop', async () => {
      await scoped.workOrder.update({ where: { id: 'wo-1' }, data: { status: 'LISTO' } });
  ...
  describe('cross-workshop isolation', () => {
    it('two scoped instances never share workshopId', async () => {
      ...
      const [callA, callB] = (prisma.customer.findMany as jest.Mock).mock.calls;
    ...
    it('workOrder.create from workshop A never writes workshopId B', async () => {
      ...
      await scoped.workOrder.create({ data: { vehicleId: 'v', customerId: 'c' } });
      await scopedB.workOrder.create({ data: { vehicleId: 'v', customerId: 'c' } });

      const [callA, callB] = (prisma.workOrder.create as jest.Mock).mock.calls;
